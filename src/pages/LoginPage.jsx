...
const LoginPage = () => {
  ...
  const { control, handleSubmit, formState: { errors } } = useForm({
    defaultValues: { email: 'admin@easylearn.app', password: 'admin123' },
  })
    ...
    <div className="auth-page">
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ background: 'var(--surface-card)', border: '1px solid var(--surface-border)', borderRadius: 12, padding: '2rem' }}>
          <div style={{ fontWeight: 700, fontSize: '1.1rem', marginBottom: '0.25rem' }}>Sign in to your account</div>
          <div style={{ fontSize: '0.875rem', color: 'var(--text-color-secondary)', marginBottom: '1.5rem' }}>Enter your credentials to continue</div>

          {error && <Message severity="error" text={error} style={{ width: '100%', marginBottom: '1rem' }} />}

          <form onSubmit={handleSubmit(async (d) => { await login(d) })} noValidate>
              ...
              <Controller
                ...
                render={({ field: f }) => (
                  <div style={{ position: 'relative' }}>
                    ...
                    <InputText {...f} type="email" autoComplete="email" {...field('Email', errors.email)} style={{ width: '100%', paddingLeft: '2.25rem' }} />
                  </div>
                )}
              />
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <Controller
                ...
                render={({ field: f }) => (
                  <Password {...f} feedback={false} toggleMask autoComplete="current-password" className={errors.password ? 'p-invalid' : ''} />
                )}
              />
